  ...
  for (int b = 0; b < batches; ++b) {
    for (int out_c = 0; out_c < output_depth; ++out_c) {
      ...
      for (int d = 0; d < accum_depth; ++d) {
        // int32_t input_val = input_data[b * accum_depth + d];//I revise
        int32_t input_val =
          input_data[Offset(input_shape, b,0,0, d)];  
        // int32_t filter_val = filter_data[out_c * accum_depth + d]; // I revise
        int32_t filter_val = filter_data[Offset(filter_shape,0,0, out_c, d)];
      ...
      output_data[Offset(output_shape,b,0,  out_c,0)] = static_cast<int8_t>(acc);
    }
  }
}
