...
inline void ConvPerChannel(
  ...
  if (pad_width!=1 || pad_height!=1) {
    ...
    //cfu_op1 -> reset all,and send K_
    //cfu_op2 -> send data
    //cfu_op3 -> take data
    //cfu_op4 -> reset data
    //cfu_op5 -> send buf_idx(no need)
    //cfu_op6 -> send signal which means data transfer is done
    //cfu_op7 -> check whether calculation is done

    
    uint8_t matrix_a[4096][4096];  
    int8_t matrix_b[4096][4096];
    int32_t matrix_out[4096][4096];  

    const int filter_element_num = filter_height * filter_width;
    //store input matrix
    int matrix_a_yCount = 0;
    int element_count_a = 0;
    for (int out_y = 0; out_y < output_height; out_y++){
      for (int out_x = 0; out_x < output_width; out_x++){
        element_count_a = 0; //count how many element in filter has been stored.
        for (int filter_y = 0; filter_y < filter_height; filter_y++){
          for (int filter_x = 0; filter_x < filter_width; filter_x++){
            for (int in_channel = 0; in_channel < input_depth; in_channel++){
              int input_val = input_data[Offset(input_shape, 0, out_y + filter_y, out_x + filter_x, in_channel)];
              matrix_a[matrix_a_yCount][element_count_a + in_channel * filter_element_num] = input_val;
              // matrix_a[matrix_a_yCount][element_count_a + in_channel * filter_element_num] = input_val + input_offset;
            }
            element_count_a++;
          }
        }
    ...
    for (int out_channel = 0; out_channel < output_depth; out_channel++){
      ...
      for (int filter_y = 0; filter_y < filter_height; filter_y++){
        for (int filter_x = 0; filter_x < filter_width; filter_x++){
          for (int filter_channel = 0; filter_channel < filter_input_depth; filter_channel++){
            int filter_val = filter_data[Offset(filter_shape, out_channel, filter_y, filter_x, filter_channel)];
            matrix_b[element_count_b + filter_channel][out_channel] = filter_val; 
          }  
          element_count_b++;
        }  
      }
    ...
    // const int matrix_a_row = output_height * output_width;
    // const int matrix_a_col = filter_height * filter_width * input_depth;
    // const int matrix_b_col = output_depth;

    // for (int m = 0; m < matrix_a_row; m++) {
    //     for (int n = 0; n < matrix_b_col; n++) {
    //       int32_t acc = 0;
    //       for (int k = 0; k < matrix_a_col; k++) {
    //         acc += matrix_a[m][k] * matrix_b[k][n];
    //       }
    //       matrix_out[m][n] = acc;
    //     }
    // }
    ...
    //reset all
    cfu_op1(0,0,0);
    cfu_op1(0,matrix_a_cols,input_offset); //send offset to cfu
    ...
    //ensure that storing c buffer proccess is done
    while(1){
      int tmp = cfu_op7(0,0,0);
      printf("tmp: %d", tmp);
      if(tmp>=(count-1)){
        break;
      }
      ...
      for(int j=0 ; j<matrix_b_cols ; j = j+4){
          for(int idx = 0; idx<4 ; idx++){
            for(int idy = 0; idy < 4; idy++){
                ...
                matrix_out[i+idx][j+idy] = result_tmp;
      ...
      for (int out_x = 0; out_x < output_width; out_x++) {
          for (int out_channel = 0; out_channel < output_depth; out_channel++) {
              ...
              /*     11        11 11
                    ...
                    9
                    4                      */
              int32_t acc = matrix_out[out_x + out_y * output_width][out_channel]; //column of output feature maps is exactly out_channel
