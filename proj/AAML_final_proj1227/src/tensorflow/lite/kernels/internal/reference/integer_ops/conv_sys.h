...
inline void ConvPerChannel(
  ...
  int32_t matrix_a[4096][1024]; //uint8_t -> int8_t
  int32_t matrix_b[1024][1024];
  ...
  for (int batch = 0; batch < batches; ++batch) {
    for (int out_y = 0; out_y < output_height; ++out_y) {
      ...
      for (int out_x = 0; out_x < output_width; ++out_x) {
        ...
        int matrix_a_row = out_y * output_width + out_x; // row index
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              for (int in_channel = 0; in_channel < filter_input_depth;
                   ++in_channel) {
                    ...
                    // if((input_val)<-128 || input_val >127){
                    //   printf("error val = %ld\n",input_val);
                    // }                    
              }
            }
  ...
  for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
    ...
    int32_t tmp_off = cfu_op1(0,matrix_a_cols,input_offset);
    printf("tmp = %ld\n",tmp_off);
    // printf("offset = %ld",input_offset);
    for(int i=0; i<matrix_a_rows ; i = i+4){
      for(int j=0 ; j<matrix_b_cols ; j = j+4){
          ...
          //first data
            int8_t ainput0 = matrix_a[i][k];
            // if()
            int8_t ainput1 = (i+1<matrix_a_rows)? matrix_a[i+1][k] : 0;
            int8_t ainput2 = (i+2<matrix_a_rows)? matrix_a[i+2][k] : 0;
            int8_t ainput3 = (i+3<matrix_a_rows)? matrix_a[i+3][k] : 0;
            int32_t ainput = (ainput0<<24)|(ainput1<<16)|(ainput2<<8)|(ainput3);

            int8_t binput0 = matrix_b[k][j];
            int8_t binput1 = (j+1<matrix_b_cols)? matrix_b[k][j+1] : 0;
            int8_t binput2 = (j+2<matrix_b_cols)? matrix_b[k][j+2] : 0;
            int8_t binput3 = (j+3<matrix_b_cols)? matrix_b[k][j+3] : 0;
            int32_t binput = (binput0<<24)|(binput1<<16)|(binput2<<8)|(binput3);
