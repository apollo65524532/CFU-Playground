...
#define MAX_CHANNEL_SIZE 4096
#define MAX_HWC_SIZE 4096
...
inline void ConvPerChannel(
    ...
    TFLITE_DCHECK_LE(HWC, MAX_HWC_SIZE);
    TFLITE_DCHECK_LE(max_window_sliding_time, MAX_WINDOW_COUNT);
    TFLITE_DCHECK_LE(filter_number, MAX_CHANNEL_SIZE);

    for (int batch = 0; batch < batches; ++batch) {
        // initialize the im2col matrices
        int8_t weight_im2col[MAX_CHANNEL_SIZE][MAX_HWC_SIZE] = {0};
        int8_t input_im2col[MAX_HWC_SIZE][MAX_WINDOW_COUNT] = {0};
        int32_t result_im2col[MAX_CHANNEL_SIZE][MAX_WINDOW_COUNT] = {0};
                        ...
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            ...
                            for (int in_channel = 0; in_channel < filter_input_depth;
                                ...
                                if (in_x < 0 || in_x >= input_width || in_y < 0 || in_y >= input_height) {
                                    ...
                                    input_im2col[hwc_index][window_index] = input_val;
                                }
                            }
                        }
                    }
        ...
        // reshape filter to 2D (N, HxWxC)x
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
            for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
        ...
        const int M = filter_number;
        const int N = max_window_sliding_time;
        const int K = HWC;
        // Original GEMM operation
        for (int k = 0; k < K; ++k) {
            for (int m = 0; m < M; ++m) {
                for (int n = 0; n < N; ++n) {
                    result_im2col[m][n] += (int32_t)weight_im2col[m][k] * ((int32_t)input_im2col[k][n] + input_offset);
                }
            }
        }
